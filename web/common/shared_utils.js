...
export function defineProperty(instance, property, desc) {
    ...
    if ((existingDesc === null || existingDesc === void 0 ? void 0 : existingDesc.configurable) === false) {
        throw new Error(`Error: ComfyUI-Compare cannot define un-configurable property "${property}"`);
    }
...
export class Broadcaster extends EventTarget {
    ...
    openWindowAndWaitForMessage(path, windowName) {
        const id = this.getId();
        this.queue[id] = getResolver();
        const win = window.open(`${path}#broadcastLoadMsgId=${id}`, windowName);
    ...
    onMessage(e) {
        ...
        if (queueItem) {
            ...
            this.dispatchEvent(new CustomEvent("compare-broadcast-message", {
                detail: Object.assign({ replyTo: (_b = e.data) === null || _b === void 0 ? void 0 : _b.id }, e.data),
            }));
        }
    }
    addMessageListener(callback, options) {
        return super.addEventListener("compare-broadcast-message", callback, options);
    }
}
