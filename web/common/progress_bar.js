...
export class CompareProgressBar extends HTMLElement {
    static create() {
        return document.createElement(CompareProgressBar.NAME);
    }
    get currentNodeId() {
        var _a, _b;
    ...
    connectedCallback() {
        ...
        this.shadow = this.attachShadow({ mode: "open" });
            ...
            :host {
                ...
                background: var(--compare-progress-bg-color);
                --compare-progress-bg-color: rgba(23, 23, 23, 0.9);
                --compare-progress-nodes-bg-color: rgb(0, 128, 0);
                --compare-progress-steps-bg-color: rgb(0, 128, 0);
                --compare-progress-error-bg-color: rgb(128, 0, 0);
                --compare-progress-text-color: #fff;
            }
      :host * {
        box-sizing: inherit;
      }
            ...
            :host > div.bar {
                background: var(--compare-progress-nodes-bg-color);
        position: absolute;
        left: 0;
        top: 0;
        width: 0%;
        ...
        transition: width 50ms ease-in-out;
            ...
            :host > div.bar + div.bar {
                background: var(--compare-progress-steps-bg-color);
        top: 50%;
        height: 50%;
        z-index: 2;
      }
            :host > div.bar.-error {
                background: var(--compare-progress-error-bg-color);
      }
      ...
      :host > span {
        ...
        justify-content: start;
                color: var(--compare-progress-text-color);
...
CompareProgressBar.NAME = "compare-progress-bar";
customElements.define(CompareProgressBar.NAME, CompareProgressBar);
