...
export let logoCompare = "";
export async function logoCompareAsync() {
  var _a;
  if (logoCompare)
    return logoCompare;
  const baseUrl = "./";
  return fetch(`${baseUrl}logo_markup.svg?fg=currentColor&cssClass=compare-logo&w=auto&h=auto`)
    .then((r) => r.text())
    .then((t) => {
    if (t.length < 100) {
      t = `<svg viewBox="0 0 256 256" fill="currentColor" class="compare-logo">
