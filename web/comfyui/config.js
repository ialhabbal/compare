...
import { CompareDialog } from "../common/dialog.js";
import { createElement as $el, queryAll as $$ } from "../common/utils_dom.js";
import { checkmark, logoCompare } from "../common/media/svgs.js";
import { compare } from "./mynode.js";
    ...
    menus: [
        {
            ...
            description: "Have quick access from ComfyUI's new top bar to compare-comfy bookmarks, settings " +
                "(and more to come).",
        },
        {
        ...
        },
        {
            ...
            description: "Will list bookmarks in the compare-comfy right-click context menu.",
        },
    ],
    groups: [
        {
    ...
    advanced: [
        ...
        },
        {
            ...
            onSave: function (value) {
                compare.setLogLevel(value);
            },
        },
        {
            key: "features.invoke_extensions_async.node_created",
            type: ConfigType.BOOLEAN,
            label: "Allow other extensions to call nodeCreated on compare-nodes.",
            isDevOnly: true,
            description: "Do not disable unless you are having trouble (and then file an issue at compare-comfy)." +
                "Prior to Apr 2024 it was not possible for other extensions to invoke their nodeCreated " +
                "event on some compare-comfy nodes. Now it's possible and this option is only here in " +
...
export class CompareConfigDialog extends CompareDialog {
    constructor() {
        const content = $el("div");
        content.appendChild(CompareConfigDialog.buildFieldset(CONFIGURABLE["features"], "Features"));
        content.appendChild(CompareConfigDialog.buildFieldset(CONFIGURABLE["menus"], "Menus"));
        content.appendChild(CompareConfigDialog.buildFieldset(CONFIGURABLE["groups"], "Groups"));
        content.appendChild(CompareConfigDialog.buildFieldset(CONFIGURABLE["advanced"], "Advanced"));
        ...
        const dialogOptions = {
            ...
            title: logoCompare + `<h2>Settings - compare-comfy</h2>`,
            content,
            onBeforeClose: () => {
                const changed = this.getChangedFormData();
            ...
            buttons: [
                {
                    label: "Save",
                    disabled: true,
                    className: "compare-button save-button -blue",
                    callback: async (e) => {
                        var _a, _b;
                        const changed = this.getChangedFormData();
                        if (!Object.keys(changed).length) {
                        ...
                        if (success) {
                            ...
                            compare.showMessage({
                                id: "config-success",
                                message: `${checkmark} Successfully saved compare-comfy settings!`,
                                timeout: 4000,
                            });
                            $$(".save-button", this.element)[0].disabled = true;
                        }
                        else {
                            alert("There was an error saving compare-comfy configuration.");
                        }
                    },
                },
            ],
    ...
    static buildFieldset(datas, label) {
        ...
        for (const data of datas) {
            if (data.isDevOnly && !compare.isDevMode()) {
                continue;
            }
    ...
    getChangedFormData() {
        return $$("[data-name]", this.contentElement).reduce((acc, el) => {
            ...
            let currentValueEl = $$("fieldset.compare-checklist-group, input, textarea, select", el)[0];
            ...
            if (type === String(ConfigType.BOOLEAN)) {
                ...
                else if (currentValueEl.classList.contains('compare-checklist-group')) {
                    currentValue = [];
...
app.ui.settings.addSetting({
    id: "compare.config",
    defaultValue: null,
    name: "Open compare-comfy config",
    type: () => {
        return $el("tr.compare-comfyui-settings-row", {
            children: [
                $el("td", {
                    child: `<div>${logoCompare} [compare-comfy] configuration / settings</div>`,
                }),
                $el("td", {
                    child: $el('button.compare-button.-blue[text="compare-comfy settings"]', {
                        events: {
                            click: (e) => {
                                new CompareConfigDialog().show();
                            },
                        },
                    }),
                }),
