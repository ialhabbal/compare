...
import { LogLevel, compare } from "./mynode.js";
import { addHelpMenuItem } from "./utils.js";
import { CompareHelpDialog } from "../../common/dialog.js";
import { importIndividualNodesInnerOnDragDrop, importIndividualNodesInnerOnDragOver, } from "./feature_import_individual_nodes.js";
import { defineProperty, moveArrayItem } from "../../common/shared_utils.js";
export class CompareBaseNode extends LGraphNode {
    constructor(title = CompareBaseNode.title, skipOnConstructedCall = true) {
        super(title);
        this.comfyClass = "__NEED_COMFY_CLASS__";
        this.nickname = "compare";
        ...
        if (title == "__NEED_CLASS_TITLE__") {
            throw new Error("CompareBaseNode needs overrides.");
        }
        this.widgets = this.widgets || [];
        this.properties = this.properties || {};
        setTimeout(() => {
            if (this.comfyClass == "__NEED_COMFY_CLASS__") {
                throw new Error("CompareBaseNode needs a comfy class override.");
            }
        ...
        defineProperty(this, "mode", {
            ...
            set: (mode) => {
                if (this.compare_mode != mode) {
                    const oldMode = this.compare_mode;
                    this.compare_mode = mode;
    ...
    checkAndRunOnConstructed() {
        ...
        if (!this.__constructed__) {
            this.onConstructed();
                    const [n, v] = compare.logger.logParts(LogLevel.DEV, `[CompareBaseNode] Child class did not call onConstructed for "${this.type}.`);
    ...
    onConstructed() {
        ...
        compare.invokeExtensionsAsync("nodeCreated", this);
        return this.__constructed__;
    }
    configure(info) {
    ...
    showHelp() {
        ...
        if (help) {
            this.helpDialog = new CompareHelpDialog(this, help).show();
...
CompareBaseNode.exposedActions = [];
CompareBaseNode.title = "__NEED_CLASS_TITLE__";
CompareBaseNode.type = "__NEED_CLASS_TYPE__";
CompareBaseNode.category = "compare";
CompareBaseNode._category = "compare";
export class CompareBaseVirtualNode extends CompareBaseNode {
    constructor(title = CompareBaseNode.title) {
        super(title, false);
        this.isVirtualNode = true;
    }
    static setUp() {
        if (!this.type) {
            throw new Error(`Missing type for CompareBaseVirtualNode: ${this.title}`);
        }
...
export class CompareBaseServerNode extends CompareBaseNode {
    constructor(title) {
        super(title, true);
    ...
    static registerForOverride(comfyClass, nodeData, compareClass) {
        ...
        OVERRIDDEN_SERVER_NODES.set(comfyClass, compareClass);
        if (!compareClass.__registeredForOverride__) {
            compareClass.__registeredForOverride__ = true;
            compareClass.nodeType = comfyClass;
            compareClass.nodeData = nodeData;
            compareClass.onRegisteredForOverride(comfyClass, compareClass);
        }
    }
    static onRegisteredForOverride(comfyClass, compareClass) {
    }
}
CompareBaseServerNode.nodeType = null;
CompareBaseServerNode.nodeData = null;
CompareBaseServerNode.__registeredForOverride__ = false;
...
LiteGraph.registerNodeType = async function (nodeId, baseClass) {
    ...
    if (clazz !== baseClass) {
        ...
        const [n, v] = compare.logger.logParts(LogLevel.DEBUG, `${nodeId}: replacing default ComfyNode implementation with custom ${classLabel} class.`);
        (_a = console[n]) === null || _a === void 0 ? void 0 : _a.call(console, ...v);
    }
