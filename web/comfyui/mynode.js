...
import { NodeTypesString, addCompare, getNodeTypeStrings } from "./constants.js";
import { CompareProgressBar } from "../common/progress_bar.js";
import { CompareConfigDialog } from "./config.js";
import { iconGear, iconNode, iconReplace, iconStarFilled, logoCompare, } from "../common/media/svgs.js";
import { createElement, queryAll, query } from "../common/utils_dom.js";
export var LogLevel;
(function (LogLevel) {
...
const INVOKE_EXTENSIONS_BLOCKLIST = [
    {
        ...
        reason: "Major conflict with compare-comfy nodes' inputs causing instability and " +
...
class LogSession {
    ...
    logParts(level, message, ...args) {
        ...
        return this.logger.logParts(level, message, ...args);
                        ...
                        const [n, v] = compare.logParts(LogLevel.WARN, `The workflow you've loaded has corrupt linking data. Open ${new URL(location.href).origin}/compare/link_fixer to try to fix.`);
        message = `${this.name || ""}${message ? " " + message : ""}`;
    ...
    warnParts(message, ...args) {
                                                        compare.hideMessage("bad-links");
    }
    errorParts(message, ...args) {
        return this.logParts(LogLevel.ERROR, message, ...args);
    }
                                                            compare.monitorLinkTimeout = setTimeout(() => {
                                                                compare.monitorBadLinks();
    }
}
class Compare extends EventTarget {
    constructor() {
        var _a, _b, _c, _d;
        super();
        this.api = api;
        ...
        this.logger = new LogSession("[compare]");
        ...
        this.compareCssPromise = injectCss("extensions/compare-comfy/compare.css");
        ...
        CONFIG_SERVICE.addEventListener("config-change", ((e) => {
            ...
            const elDebugKeydowns = createElement("div.compare-debug-keydowns", {
    ...
    async initializeProgressBar() {
        ...
        if (CONFIG_SERVICE.getConfigValue("features.progress_bar.enabled")) {
            await this.compareCssPromise;
            if (!this.progressBarEl) {
                this.progressBarEl = CompareProgressBar.create();
                this.progressBarEl.setAttribute("title", "Progress Bar by compare. right-click for compare menu.");
                ...
                this.progressBarEl.addEventListener("pointerdown", async (e) => {
                    ...
                    if (e.button == 2) {
                        ...
                        new LiteGraph.ContextMenu(this.getCompareContextMenuValues(), {
                            title: `<div class="compare-contextmenu-item compare-contextmenu-title-compare-comfy">${logoCompare} compare-comfy</div>`,
                            left: e.clientX,
                            top: 5,
                        });
                        return;
            ...
            this.progressBarEl.classList.toggle("compare-pos-bottom", position === "bottom");
    ...
    async initializeGraphAndCanvasHooks() {
        ...
        LGraph.prototype.serialize = function () {
            ...
            compare.initialGraphToPromptSerializedWorkflowBecauseComfyUIBrokeStuff = response;
            return response;
        };
        const processMouseDown = LGraphCanvas.prototype.processMouseDown;
        LGraphCanvas.prototype.processMouseDown = function (e) {
            compare.processingMouseDown = true;
            const returnVal = processMouseDown.apply(this, [...arguments]);
            compare.dispatchCustomEvent("on-process-mouse-down", { originalEvent: e });
            compare.processingMouseDown = false;
            return returnVal;
        };
        const adjustMouseEvent = LGraphCanvas.prototype.adjustMouseEvent;
        LGraphCanvas.prototype.adjustMouseEvent = function (e) {
            adjustMouseEvent.apply(this, [...arguments]);
            compare.lastCanvasMouseEvent = e;
        };
        const copyToClipboard = LGraphCanvas.prototype.copyToClipboard;
        LGraphCanvas.prototype.copyToClipboard = function (items) {
            compare.canvasCurrentlyCopyingToClipboard = true;
            compare.canvasCurrentlyCopyingToClipboardWithMultipleNodes =
                Object.values(items || this.selected_nodes || []).length > 1;
            const value = copyToClipboard.apply(this, [...arguments]);
            compare.canvasCurrentlyCopyingToClipboard = false;
            compare.canvasCurrentlyCopyingToClipboardWithMultipleNodes = false;
            return value;
        };
        ...
        LGraphCanvas.prototype.pasteFromClipboard = function (...args) {
            compare.canvasCurrentlyPastingFromClipboard = true;
            pasteFromClipboard.apply(this, [...arguments]);
            compare.canvasCurrentlyPastingFromClipboard = false;
        };
    ...
    async invokeExtensionsAsync(method, ...args) {
        ...
        if (CONFIG_SERVICE.getConfigValue("features.invoke_extensions_async.node_created") === false) {
                    const [m, a] = this.logParts(LogLevel.INFO, `Skipping invokeExtensionsAsync for applicable compare-comfy nodes`);
        ...
        return await Promise.all(comfyapp.extensions.map(async (ext) => {
            ...
            if (ext === null || ext === void 0 ? void 0 : ext[method]) {
                try {
                    ...
                    if (blocked) {
                            const [n, v] = this.logger.logPartsOnceForTime(LogLevel.WARN, 5000, `Blocked extension '${ext.name}' method '${method}' for compare-nodes because: ${blocked.reason}`);
                        (_a = console[n]) === null || _a === void 0 ? void 0 : _a.call(console, ...v);
                        return Promise.resolve();
                    }
                    ...
                    const [n, v] = this.logParts(LogLevel.ERROR, `Error calling extension '${ext.name}' method '${method}' for compare-node.`, { error }, { extension: ext }, { args });
    ...
    async initializeContextMenu() {
        ...
        setTimeout(async () => {
            ...
            LGraphCanvas.prototype.getCanvasMenuOptions = function (...args) {
                ...
                options.push(null);
                options.push({
                    content: logoCompare + `compare-comfy`,
                    className: "compare-contextmenu-item compare-contextmenu-main-item-compare-comfy",
                    submenu: {
                        options: that.getCompareContextMenuValues(),
                    },
                });
                options.push(null);
                options.push(null);
        ...
        return [
            {
                content: "Nodes",
                disabled: true,
                className: "compare-contextmenu-item compare-contextmenu-label",
            },
            {
                content: iconNode + "All",
                    className: "compare-contextmenu-item",
                has_submenu: true,
                submenu: {
                    options: getNodeTypeStrings(),
                            callback: (value, options, event) => {
                        const node = LiteGraph.createNode(addCompare(value));
                        if (node) {
                            node.pos = [
                                compare.lastCanvasMouseEvent.canvasX,
                                compare.lastCanvasMouseEvent.canvasY,
                            ];
            ...
            {
                    content: iconGear + "Settings (compare-comfy)",
                disabled: !!this.settingsDialog,
                    className: "compare-contextmenu-item",
                callback: (...args) => {
                    this.settingsDialog = new CompareConfigDialog().show();
            ...
            },
            {
                ...
                className: "compare-contextmenu-item",
                callback: (...args) => {
                    const msg = `Convert ${rerouteLabel} ComfyUI Reroutes to Reroute (compare) nodes? \n` +
            ...
            },
            {
                ...
                className: "compare-contextmenu-item compare-contextmenu-github",
                callback: (...args) => {
                    window.open("https://github.com/ComfyUI/ComfyUI-Compare", "_blank");
                },
            },
        ];
    }
    ...
    initializeComfyUIHooks() {
        ...
        app.queuePrompt = async function (number, batchCount) {
            compare.processingQueue = true;
            compare.dispatchCustomEvent("queue");
            try {
                return await queuePrompt.apply(app, [...arguments]);
            }
            finally {
                compare.processingQueue = false;
                compare.dispatchCustomEvent("queue-end");
            }
        };
        const loadApiJson = app.loadApiJson;
        app.loadApiJson = async function (apiData, fileName) {
            compare.loadingApiJson = apiData;
            try {
                loadApiJson.apply(app, [...arguments]);
            }
            finally {
                compare.loadingApiJson = null;
            }
        };
        const graphToPrompt = app.graphToPrompt;
        app.graphToPrompt = async function () {
            compare.dispatchCustomEvent("graph-to-prompt");
            let promise = graphToPrompt.apply(app, [...arguments]);
            await promise;
            compare.dispatchCustomEvent("graph-to-prompt-end");
            return promise;
        };
        const apiQueuePrompt = api.queuePrompt;
        api.queuePrompt = async function (index, prompt, ...args) {
            var _a;
            if (((_a = compare.queueNodeIds) === null || _a === void 0 ? void 0 : _a.length) && prompt.output) {
                const oldOutput = prompt.output;
                let newOutput = {};
                for (const queueNodeId of compare.queueNodeIds) {
                    compare.recursiveAddNodes(String(queueNodeId), oldOutput, newOutput);
                }
                prompt.output = newOutput;
            }
            compare.dispatchCustomEvent("comfy-api-queue-prompt-before", {
                workflow: prompt.workflow,
                output: prompt.output,
            });
            const response = apiQueuePrompt.apply(app, [index, prompt, ...args]);
            compare.dispatchCustomEvent("comfy-api-queue-prompt-end");
            return response;
        };
        const clean = app.clean;
        app.clean = function () {
            compare.clearAllMessages();
            clean && clean.apply(app, [...arguments]);
        };
        const loadGraphData = app.loadGraphData;
        app.loadGraphData = function (graph) {
            if (compare.monitorLinkTimeout) {
                clearTimeout(compare.monitorLinkTimeout);
                compare.monitorLinkTimeout = null;
            }
            compare.clearAllMessages();
            let graphCopy;
            try {
    ...
    async showMessage(data) {
        let container = document.querySelector(".compare-top-messages-container");
        if (!container) {
            container = document.createElement("div");
            container.classList.add("compare-top-messages-container");
    ...
    async hideMessage(id) {
        const msg = document.querySelector(`.compare-top-messages-container > [msg-id="${id}"]`);
    ...
    isDebugMode() {
        if (window.location.href.includes("compare-debug=false")) {
            return false;
        }
        return GLOBAL_LOG_LEVEL >= LogLevel.DEBUG || window.location.href.includes("compare-debug");
    }
    isDevMode() {
        if (window.location.href.includes("compare-dev=false")) {
            return false;
        }
        return GLOBAL_LOG_LEVEL >= LogLevel.DEV || window.location.href.includes("compare-dev");
    }
    monitorBadLinks() {
        ...
        if (badLinksFound.hasBadLinks && !this.monitorBadLinksAlerted) {
            ...
            alert(`Problematic links just found in live data. Can you save your workflow and file a bug with ` +
                ...
                `https://github.com/ComfyUI/ComfyUI-Compare/issues. Thank you!`);
        }
...
app.registerExtension({
    ...
    aboutPageBadges: [
        {
            label: `ComfyUI-Compare v${compare.version}`,
            url: "https://github.com/ComfyUI/ComfyUI-Compare",
            icon: "comfyui-compare-about-badge-logo",
        },
    ],
});
