...
import { getResolver, wait } from "../../common/shared_utils.js";
...
export function addMenuItemOnExtraMenuOptions(node, config, menuOptions, after = "Shape") {
    let idx = menuOptions
        .slice()
        .reverse()
        .findIndex((option) => option === null || option === void 0 ? void 0 : option.isCompare);
    if (idx == -1) {
    ...
    addMenuItemOnExtraMenuOptions(node, {
        ...
        callback: (node) => {
            if (node.showHelp) {
                ...
                new CompareHelpDialog(node, content).show();
            }
        },
